...
Lake Loader 0.10.0 supports [Delta](https://docs.delta.io/latest/index.html) and [Iceberg](https://iceberg.apache.org/docs/latest/) as output formats.
...
docker run \
  ...
  snowplow/lake-loader-azure:0.10.0 \
...
docker run \
  -v /path/to/config.hocon:/var/config.hocon \
  -v /path/to/iglu.json:/var/iglu.json \
  snowplow/lake-loader-gcp:0.10.0 \
  --config /var/config.hocon \
  --iglu-config /var/iglu.json
```
...
docker run \
  -v /path/to/config.hocon:/var/config.hocon \
  -v /path/to/iglu.json:/var/iglu.json \
  snowplow/lake-loader-aws:0.10.0 \
  --config /var/config.hocon \
  --iglu-config /var/iglu.json
```
