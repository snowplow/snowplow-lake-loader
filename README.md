...
Lake Loader 0.6.3 supports [Delta](https://docs.delta.io/latest/index.html), [Iceberg](https://iceberg.apache.org/docs/latest/) and [Hudi](https://hudi.apache.org/docs/overview/) as output formats.
...
docker run \
  ...
  snowplow/lake-loader-azure:0.6.3 \
...
docker run \
  -v /path/to/config.hocon:/var/config.hocon \
  -v /path/to/iglu.json:/var/iglu.json \
  snowplow/lake-loader-gcp:0.6.3 \
  --config /var/config.hocon \
  --iglu-config /var/iglu.json
```
...
docker run \
  -v /path/to/config.hocon:/var/config.hocon \
  -v /path/to/iglu.json:/var/iglu.json \
  snowplow/lake-loader-aws:0.6.3 \
  --config /var/config.hocon \
  --iglu-config /var/iglu.json
```
...
[release-image]: https://img.shields.io/badge/release-0.6.3-blue.svg?style=flat
